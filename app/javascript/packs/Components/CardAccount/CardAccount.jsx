...
const CardAccount = ({id, name, amount, currency, img=DEFAULT_IMAGE})=> {
  return (
    <Link to={`${id}`} className="bg-white p-3 w-full flex flex-col rounded-md dark:bg-gray-800 shadow">
      ...
      <div classnames="flex items-center w-full">
        <div classnames="text-xs py-1 px-2 leading-none dark:bg-gray-900 bg-blue-100 text-blue-500 rounded-md">{currency?.code}</div>
        <div classnames="ml-auto text-xs text-gray-500">{currency?.symbol} {amount}</div>
      </div>
    </Link>
  );
};
